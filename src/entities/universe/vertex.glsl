...
vec3 getOffset(vec3 p) {
  ...
  vec3 temppos = rotation3dY(time*(0.1 + 0.5*twist_scale) + length(pos.xy))*p;
...
void main() {
  ...
  float distanceToMouse = pow(1. - clamp(length(uMouse.xz - particle_position.xz)-0.5, 0., 1.), 4.);
  ...
  view_pos.xyz += position*size*(0.12+ 0.1*particle_size);
